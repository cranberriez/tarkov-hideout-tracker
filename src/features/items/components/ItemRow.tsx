...
export function ItemRow({ item, count, firCount = 0, compact, sellToPreference = "best", onClick }: ItemRowProps) {
			...
			>
				...
				<div className="flex-1 min-w-0">
					<div className="flex items-baseline justify-between">
						<span className="text-sm font-medium text-gray-200 truncate mr-2" title={item.name}>
							{item.name}
						</span>
		...
		>
			...
			<div className="flex items-start gap-3 min-w-0 z-1">
				...
				<div className="min-w-0 flex-1">
					<h3 className="text-sm font-bold text-gray-100 leading-tight line-clamp-2" title={item.name}>
						{item.name}
					</h3>
			...
			<div className="grid grid-cols-2 gap-2 mt-auto pt-2 border-t border-white/5 z-1">
				...
				<div className="bg-black/30 p-1.5 rounded">
					<div className="text-[10px] text-gray-500 uppercase tracking-wide mb-0.5">Required</div>
					<div className="flex items-baseline gap-1.5">
						<div
				...
				{!isCurrency && item.avg24hPrice && (
					<div className="bg-black/30 p-1.5 rounded">
						<div className="text-[10px] text-gray-500 uppercase tracking-wide mb-0.5">Est. Cost</div>
				...
				{!isCurrency && bestSell && (
					<div className="col-span-2 bg-black/30 p-1.5 rounded flex justify-between items-center">
						<div className="text-[10px] text-gray-500 uppercase tracking-wide">
							Sell To <span className="text-gray-300 normal-case">{bestSell.vendor.name}</span>
						</div>
