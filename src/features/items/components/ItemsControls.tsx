...
export function ItemsControls({ onOpenSearch }: ItemsControlsProps) {
	...
	return (
		<div className="flex flex-col gap-2 bg-muted p-2 rounded-md border">
			...
			>
				<Search size={18} className="text-gray-500 group-hover:text-tarkov-green transition-colors" />
				Search items...
			</button>
			...
			<div className="flex flex-col xl:flex-row gap-6 justify-between items-start xl:items-center">
				...
				<div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 w-full xl:w-auto">
					...
					<ControlGroup label="View">
						...
						<ControlButton active={checklistViewMode === "all"} onClick={() => setChecklistViewMode("all")}>
							All Future
						</ControlButton>
					</ControlGroup>
					...
					<ControlGroup label="Price">
						<ControlButton active={sellToPreference === "best"} onClick={() => setSellToPreference("best")}>
							Best
						</ControlButton>
						<ControlButton active={sellToPreference === "flea"} onClick={() => setSellToPreference("flea")}>
							Flea
						</ControlButton>
						<ControlButton
...
function ControlGroup({ label, children }: { label: string; children: ReactNode }) {
	return (
		<div className="flex flex-col gap-1.5">
			<span className="text-[10px] uppercase tracking-wider text-gray-500 font-bold pl-1">{label}</span>
	...
	return (
		<button
			...
			className={`flex items-center gap-2 px-3 py-1.5 text-xs font-medium rounded-xs transition-all ${
				active ? "bg-tarkov-green text-black shadow-sm" : "text-gray-400 hover:text-white hover:bg-white/5"
			}`}
		>
			{icon}
			{children}
