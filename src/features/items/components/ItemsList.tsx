...
export function ItemsList({ onClickItem }: ItemsListProps) {
	const { stations, fetchStations, items, fetchItems, loadingStations, loadingItems, errorStations, errorItems } =
		useDataStore();
	...
	if (errorStations || errorItems) {
		return <div className="text-center text-red-500 py-10">Error: {errorStations || errorItems}</div>;
	}
	...
	if (filteredAndSortedItems.length === 0) {
		return (
			<div className="text-center text-gray-500 py-20">
				...
				<div className="text-sm">You might have maxed out your hideout or hidden all stations.</div>
			</div>
		);
	}
