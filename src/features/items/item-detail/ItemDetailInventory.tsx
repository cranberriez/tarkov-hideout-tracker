...
export function ItemDetailInventory({
    ...
    return (
        <ItemDetailSection title="Inventory">
            <div className="space-y-1 bg-shadow/10">
            ...
            {hasChanges && (
                <div className="mt-4 flex items-center justify-end gap-2 border-t border-border-color pt-4">
                    <button
                        type="button"
                        onClick={onReset}
                        className="inline-flex h-8 items-center gap-1.5 rounded-md px-2.5 text-xs text-muted-foreground transition-colors hover:bg-highlight/5 hover:text-foreground"
                    >
                        <RotateCcw size={13} />
                        Reset
                    </button>
                    <button
                        type="button"
                        onClick={onSave}
                        className="inline-flex h-8 items-center gap-1.5 rounded-md border border-brand/35 bg-brand/10 px-3 text-xs font-medium text-brand transition-colors hover:bg-brand/20"
                    >
                        <Check size={14} />
                        Save inventory
                    </button>
    ...
    return (
        <div className="flex items-center justify-between gap-3 px-1 py-1">
            <div className="flex items-center gap-2 pl-1 text-sm text-foreground">
                <span
                    className={`h-2 w-2 rounded-full ${fir ? "bg-warning" : "bg-success"}`}
                />
                {label}
            </div>
            <div className="flex items-center overflow-hidden rounded-md border border-border-color bg-shadow/35">
                <button
                    ...
                    className="flex h-8 w-8 items-center justify-center text-muted-foreground transition-colors hover:bg-highlight/5 hover:text-foreground"
                ...
                <input
                    ...
                    className="h-8 w-12 border-x border-border-color bg-transparent text-center font-mono text-sm font-semibold text-foreground outline-none focus:bg-highlight/5"
