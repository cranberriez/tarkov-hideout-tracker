...
export function LockReasons({ reasons, showIcon = true }: { reasons: readonly LockReason[]; showIcon?: boolean }) {
  ...
  return <span data-isolated-hover="true" className="block space-y-1 p-1 text-left text-[10px] leading-snug text-red-200">
    {uniqueReasons.map((reason, index) => <span key={`${reason.kind}:${reason.questId ?? ""}:${index}`} className="flex items-start gap-1">
      ...
      {reason.questId ? <span><span className="block">Complete Quest:</span><Link className="block underline hover:text-white" href={getQuestDeepLinkHref(reason.questId)}>{context.taskUnlocksById?.[reason.questId]?.name || "Quest details unavailable"}</Link></span> : <span>{reason.message}</span>}
    </span>)}
  </span>;
}
