...
import { PriceRefreshControl } from "@/features/items/DeferredPriceBoundary";
...
export function ProfitQueryPage({ mode, kind, initialTargetRecipeId, fallbackData }: { mode: TarkovJsonGameMode; kind: ProfitPageKind; initialTargetRecipeId?: string; fallbackData: ProfitPageData | null }) {
    ...
    return <DataQueryRetryProvider retry={() => void query.refetch()}>{query.error && !(query.error instanceof PartialDataError) && <DataRefreshError message="Updated profit data could not be loaded." />}<PriceRefreshControl delivery={prices} /><ProfitPageClient kind={kind} data={data} initialTargetRecipeId={initialTargetRecipeId} /></DataQueryRetryProvider>;
}
...
export function CraftPlannerQueryPage({ mode, fallbackData }: { mode: TarkovJsonGameMode; fallbackData: ProfitPageData | null }) {
    ...
    return <DataQueryRetryProvider retry={() => void query.refetch()}>{query.error && !(query.error instanceof PartialDataError) && <DataRefreshError message="Updated craft-planner data could not be loaded." />}<PriceRefreshControl delivery={prices} /><CraftPlannerClient data={data} /></DataQueryRetryProvider>;
}
