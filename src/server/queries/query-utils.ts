...
export function getQuestReferencedItemIds(quests: readonly FullQuest[]): string[] {
	...
	for (const quest of quests) {
		...
		for (const objective of quest.objectives) {
			...
			if ("containsAllItemIds" in objective && Array.isArray(objective.containsAllItemIds)) {
				itemIds.push(...objective.containsAllItemIds);
			}
			if ("useAnyItemIds" in objective && Array.isArray(objective.useAnyItemIds)) {
...
export function getRecipeGraphItemIds(barters: readonly BarterRecord[], crafts: readonly CraftRecord[]): string[] {
	const itemIds: string[] = [];
	...
	for (const craft of crafts) {
		...
		itemIds.push(...craft.requiredQuestItems.map((requirement) => requirement.itemId));
	}
	return dedupeIds(itemIds);
}
