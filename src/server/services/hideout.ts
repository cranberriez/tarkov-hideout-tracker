		...
		const levels = (s.levels ?? []).map((lvl): StationLevel => {
			...
			let itemRequirements = (lvl.itemRequirements ?? []).map((req): ItemRequirement => {
				...
				if (wikiLevel) {
					...
					// Fallback to config
					isFir = (requiresFoundInRaid as Record<string, Record<number, string[]>>)[s.normalizedName]?.[
						lvl.level
					]?.includes(req.item.normalizedName);
				}
			...
			if (wikiLevel) {
				...
				itemRequirements = itemRequirements.filter((req) => wikiItemNames.has(req.item.normalizedName));
			}
