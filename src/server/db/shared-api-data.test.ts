    ...
    try {
        ...
        for (const [index, mode] of (["regular", "pve", "pvp-season"] as const).entries()) {
            await db.execute({
                ...
                args: [mode, ACTIVE_DATA_RELEASE_IDS[mode], JSON.stringify({ items: 100, stations: 200, quests: 300 + index, crafts: 400 + index, barters: 500 + index })],
            });
            const changedAt = 1000 + index;
            await store.writeOutcomes(mode, [{ status: "updated", itemId: "item-a", etag: "a", checkedAt: changedAt,
                effectivePrice: 100, sampleCount: 1, totalOfferCount: 5,
            ...
            for (const [domain, expected] of [["quests", 300], ["crafts", 400], ["barters", 500]] as const) {
        ...
        await db.execute(`UPDATE data_releases SET source_freshness_json = '{"items":100,"stations":200,"quests":null,"crafts":400,"barters":"invalid"}' WHERE mode = 'pve'`);
