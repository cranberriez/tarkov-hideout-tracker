...
export async function getManifest<Payload>(
	...
	if (!row) {
		throw new TursoRecordNotFoundError(`No ${manifestName} manifest exists for ${mode}/${releaseId}`);
	}

	const updatedAt = Number(row.updated_at);
	if (!Number.isFinite(updatedAt)) {
		throw new TursoDataIntegrityError(`${manifestName} manifest has an invalid update time`);
	}

	return {
		payload: parseStoredJson<Payload>(row.payload_json, `${manifestName} manifest`),
		updatedAt,
	};
}
