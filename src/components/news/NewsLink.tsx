		...
		>
			<div className="flex items-center justify-between gap-2 mb-1">
				<div className="flex flex-col">
					<span className="text-sm font-semibold text-foreground line-clamp-1">{title}</span>
					<span className="text-[11px] font-mono text-muted-foreground">v{version}</span>
				</div>
				{date && (
					<time className="text-[11px] text-muted-foreground whitespace-nowrap">{date}</time>
				)}
			</div>
			{description && (
