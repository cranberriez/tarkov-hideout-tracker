...
import { SNAPSHOT_SCHEMA_VERSION, assertSafeReleaseId, createReleaseId, hashFile } from "./lib/snapshot.mjs";
...
function parseArguments(argv) {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		else if (argument === "--preserve-prices") options.preservePrices = true;
...
function stationPreview(station) {
	return pickDefined({
		...
		maxLevel: station.levels.reduce((highest, level) => Math.max(highest, level.level), 0),
	});
}
...
function createMemoryRepository(data) {
	...
	const currentPrices = new Map(data.items.flatMap((item) => (item.marketPrice ? [[item.id, item.marketPrice]] : [])));

	return {
		items: {
			getCatalog: async () => makeResult(data.items, data.freshness.items),
			getByIds: async (_mode, ids) => makeResult(recordsByIds(itemsById, ids), data.freshness.items),
		},
		hideout: {
		...
		quests: {
			...
			getByIds: async (_mode, ids) => makeResult(recordsByIds(questsById, ids), data.freshness.quests),
		},
		traders: {
			getAll: async () => makeResult(data.traders, data.freshness.traders),
			getByIds: async (_mode, ids) => makeResult(recordsByIds(tradersById, ids), data.freshness.traders),
		},
		recipes: {
		...
		prices: {
			getCurrent: async (_mode, ids) => makeResult(recordsByIds(currentPrices, ids), data.freshness.items),
			getHistory: async () => {
...
async function loadModeData(mode, modules) {
	const [items, skills, stations, quests, traders, barters, crafts] = await Promise.all([
...
async function writeModeSnapshot(releaseDirectory, releaseId, mode, modules, preservePrices) {
	...
	if (preservePrices) {
		...
		try {
			const previous = await readReleasePrices(client, mode);
			previousReleaseId = previous.releaseId;
			previousPrices = previous.prices;
			data.items = preserveItemPrices(data.items, previousPrices);
		} finally {
			client.close();
		}
	}
	...
	return {
		...
		recordCounts: counts,
		entityCounts: Object.fromEntries(entityGroups.map(([entityType, records]) => [entityType, records.length])),
...
async function main() {
	...
	const releaseDirectory = await prepareReleaseDirectory(options.outputRoot, options.releaseId, options.force);
	const modules = await loadApplicationModules();
	const modes = [];
	for (const mode of options.modes) {
		modes.push(await writeModeSnapshot(releaseDirectory, options.releaseId, mode, modules, options.preservePrices));
	}
	const manifest = {
