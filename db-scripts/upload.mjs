...
import { loadSnapshotManifest, readRecords, validateSnapshotFiles } from "./lib/snapshot.mjs";
import { activateRelease, applySchema, createTursoClient, statementForRecord } from "./lib/turso.mjs";

import { CURRENT_GAME_PATCH, assertGamePatch, initializeCatalogHistory, catalogPublicationStatements } from "./lib/catalog-history.mjs";
...
function parseArguments(argv) {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		if (argument === "--release-dir") {
			...
			options.patch = assertGamePatch(argv[++index]);
...
async function beginRelease(client, manifest, modeEntry) {
	...
	if (existing.rows.length > 0 && String(existing.rows[0].snapshot_sha256) !== modeEntry.sha256) {
		throw new Error(`${modeEntry.mode}/${manifest.releaseId} already exists with a different snapshot checksum`);
	}
...
async function uploadMode(client, releaseDirectory, manifest, modeEntry, batchSize, batchBytes) {
	...
	for await (const record of readRecords(filename)) {
		...
		if (statements.length > 0 && (statements.length >= batchSize || statementBytes + recordBytes > batchBytes)) {
			await flush();
		}
...
async function verifyAndMarkReady(client, manifest, modeEntry, patch) {
	const actual = {
	...
	for (const [recordType, expected] of Object.entries(modeEntry.recordCounts)) {
		if (actual[recordType] !== expected) {
			throw new Error(`${modeEntry.mode} uploaded ${actual[recordType]} ${recordType} rows; expected ${expected}`);
		}
	}
	await client.batch(catalogPublicationStatements(modeEntry.mode, manifest.releaseId, patch, Date.now()), "write");
}
...
async function main() {
	...
	try {
		...
		for (const modeEntry of manifest.modes) {
			...
			await uploadMode(client, options.releaseDirectory, manifest, modeEntry, options.batchSize, options.batchBytes);
