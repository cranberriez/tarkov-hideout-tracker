...
export async function readReleasePrices(client, mode) {
	const releaseId = await readActiveReleaseId(client, mode, { allowMissing: true });
	if (releaseId === null) return { releaseId: null, prices: new Map() };
	...
	if (!result.rows.length)
		throw new Error(`No price snapshot in ${mode}/${releaseId}; refusing to discard the existing fallback.`);
	return {
		releaseId,
		prices: new Map(
			result.rows.map((row) => [
